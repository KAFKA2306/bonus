#!/usr/bin/env python3
"""Expand the existing source-backed 30-company payload to the official Nikkei 225 universe."""
...
def fetch_constituents() -> tuple[str, list[dict[str, str]]]:
    req = urllib.request.Request(SOURCE_URL, headers={"User-Agent": "bonus-model/1.0"})
    ...
    for index, line in enumerate(lines):
        ...
        if current and re.fullmatch(r"(?:\d{4}|\d{3}[A-Z])", line):
            ...
            names = [x for x in nearby if x not in {"コード", "銘柄名", "社名"} and not re.fullmatch(r"(?:\d{4}|\d{3}[A-Z])", x)]
            if len(names) >= 2:
                rows.append({"stock_code": line, "display_name_ja": names[0], "company_name_ja": names[1], "nikkei_industry": current})
    unique = {row["stock_code"]: row for row in rows}
    if len(unique) != 225:
        raise RuntimeError(f"official Nikkei page parsed {len(unique)} constituents, expected 225")
    return as_of, sorted(unique.values(), key=lambda row: row["stock_code"])


def synthetic_record(row: dict[str, str]) -> dict:
    sector_id, sector_name, months, annual_amount, mechanism_id = SECTOR_RULES[row["nikkei_industry"]]
    label, upside, upside_score, upper_add = MECHANISMS[mechanism_id]
    low, central, high = round(max(0.5, months - 1.10), 2), round(months, 2), round(months + 1.10 + upper_add, 2)
    monthly = annual_amount / months
    amount = {key: int(round(monthly * value / 1000) * 1000) for key, value in {"minimum": low, "central": central, "maximum": high}.items()}
    broad = "performance_linked" if mechanism_id == "performance_nonformula" else ("hybrid" if mechanism_id == "hybrid" else "discretionary")
    return {
        "stock_code": row["stock_code"], "company_name_ja": row["company_name_ja"], "subject": "employees",
        "employee_scope": "個社一次資料未確認。日経業種と連合2026業種実測を用いた低信頼度の初期推定。",
        "classification": None, "evidence_status": "unknown", "as_of": "2026-08-02",
        "bonus": {"frequency_per_year": None, "annual_months": None, "pool_basis": None, "allocation_logic": None, "base_salary_link": None},
        "notes": ["日経225拡張時のセクター事前分布。個社資料取得時に置換する。"], "sources": [],
        "survey": {"stage": "queued", "open_questions": ["会社・労組一次資料で制度類型、年間月数、支給回数を確認する。"]},
        "estimate": {
            "status": "sector_prior", "method": "nikkei_industry_sector_prior", "sector_id": sector_id,
            "sector_name_ja": sector_name, "nikkei_industry": row["nikkei_industry"],
            "months": {"minimum": low, "central": central, "maximum": high}, "amount_yen": amount,
            "frequency_per_year": 2, "classification": broad,
            "mechanism": {"id": mechanism_id, "label_ja": label, "upside_profile": upside, "upside_score": upside_score, "formula_disclosure": "not_disclosed" if mechanism_id == "performance_nonformula" else "unknown", "source_url": SOURCE_URL, "source_note": "日経業種からの初期仮説であり、個社制度の確認事実ではない。"},
            "official_observations": {"annual_months": None, "latest_seasonal": None},
            "confidence": {"score": 0.30, "level": "low", "amount_score": 0.18}, "weights": {"company_prior": 0.0, "sector_actual": 1.0},
            "anchors": {"company_prior_months": {"minimum": low, "central": central, "maximum": high}, "sector_actual_months": months, "sector_actual_amount_yen": annual_amount, "sector_sample_months": {}, "source_url": SOURCE_URL},
            "override_note": None,
            "basis": [{"type": "official_index_industry", "statement": f"日経平均公式構成銘柄の業種は{row['nikkei_industry']}。", "reference": SOURCE_URL}, {"type": "official_sector_actual", "statement": f"連合2026最終集計の{sector_name}実測を初期アンカーに使用。", "reference": "https://www.jtuc-rengo.or.jp/activity/roudou/shuntou/2026/yokyu_kaito/kaito/kaito_no7_ichiji.pdf"}],
            "assumptions": ["同じ日経業種内では個社資料がない段階の中心値として業種実測を使用する。", "金額は業種平均基本月額による参考換算であり個社実額ではない。"],
            "falsifiers": ["会社・労組一次資料で制度類型または年間月数が確認された場合。"],
...
def main() -> int:
    ...
    records = [existing.get(row["stock_code"], synthetic_record(row)) for row in constituents]
    ...
    payload["universe"] = {"source_file": SOURCE_URL, "source_as_of": official_as_of, "mutation_policy": "official_snapshot_at_build", "tracked_companies": 225, "covered_companies": 225, "coverage_ratio": 1.0, "company_specific_prior_count": len(existing), "sector_prior_count": 225 - len(existing)}
    ...
    payload["summary"]["mechanism_counts"] = dict(Counter(row["estimate"]["mechanism"]["id"] for row in records))
    PAYLOAD.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Expanded Pages JSON to {len(records)} Nikkei 225 constituents (official {official_as_of})")
    return 0
